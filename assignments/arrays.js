...
let car33 = inventory.find(car33 => car33.id == 33);
...
for (i = 0; 1 < carModels.length; i++) {
    if (carModels[i] === undefined) {
        break;
    }
    let carModel = carModels[i].car_model; 
    console.log(carModel);
}

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer
// data containing only the car years and log the result in the console.
let carYears = [];
...
// you just created, find out how many cars were made before the year 2000 by populating the array oldCars and 
//logging it's length.
let oldCars = [];
for (i = 0; i < carYears.length; i++) {
    if (carYears[i] === undefined) {
        break;
    }    
    if (carYears[i] < 2000) {
...
//contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the
